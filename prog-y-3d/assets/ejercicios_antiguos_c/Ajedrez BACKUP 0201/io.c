...
CONTENIDO: Funciones de entrada/salida
           Modos soportados:  - TXT_DOS: modo texto MS-DOS (usa librerías DOS y CONIO)
	                      - TXT_LINUX: modo texto Linux (usa librería Ncurses)
                              - GRAF_LINUX: modo gráfico Linux (usa librería SDL)
           En todas las funciones se encontrará código separado para cada uno de los modos soportados
...
// Inicializa el interfaz. Invocar antes que cualquier otra función de i/o
// ---------------------------------------------------------------------------------------
...
// --------------------------------------
  ...
  keypad(stdscr, 1);		// Activamos recepción de teclas especiales
...
// --------------------------------------
  ...
  // Creamos la pantalla con doble buffer, para dibujar en el doble buffer y luego mandarlo a la memoria de vídeo
...
#ifdef TXT_LINUX
  endwin();	// En LINUX llamamos a la función de librería de Ncurses
  exit(0);
#endif
...
#endif
#ifdef GRAF_LINUX
  SDL_Flip(scr);		// Enviamos el buffer de pantalla a memoria de vídeo para visualizarlo
#endif
}
...
// --------------------------------------
  ...
  int i, j;
  y = 9-y;	// Le damos la vuelta a la fila porque en la pantalla están al revés (la 1 abajo)
...
// --------------------------------------
  ...
  // No hay que pintar la casilla (ya viene dibujada en la imagen de fondo), sólo la pieza.

  y = 9-y;	// Le damos la vuelta a la y porque en la pantalla las filas están al revés (fila 1 abajo)
  // Generamos el rectángulo adecuado para enviar el sprite a esa posición de la pantalla
  px =((x-1)*TAM_CASILLA) + DESPLZ_ORIG_X;
  py = ((y-1)*TAM_CASILLA) + DESPLZ_ORIG_Y;
  rect = (SDL_Rect) { px, py, TAM_CASILLA, TAM_CASILLA };

  // Seleccionamos el sprite adecuado según el tipo y el color de la pieza
  switch (tipo)
  {
...
// (x_orig, y_orig) son corrdenadas de origen, si es que ya se eligieron. Si no, serán (0,0)
// ---------------------------------------------------------------------------------------
...
// --------------------------------------
  ...
  // Si ya se eligió casilla de origen, empezamos la selección del destino en esas coordenadas
  ...
  do
  {
    ...
    // Si ya se eligió casilla de origen (y ahora estamos eligiendo el destino), primero mostramos el
  ...
  nocbreak();		// Volver a activar el envío habitual de teclas

  // Guardar el resultado de la selección en los parámetros pasados por referencia
  if (terminar == 1)	// Se pulsó Intro = aceptar selección
  {
    *col = x;
    *fila = 9-y;	// Recuerda: en la pantala las filas están al revés (abajo el 1)
  }
  else			// Se pulsó Esc = cancelar selección
  {
    *col = 0;
    *fila = 0;
  }
...
// --------------------------------------
#ifdef GRAF_LINUX
  int x, y;		// Coordenadas de la casilla seleccionada
  int px, py;		// Coordenadas de la casilla en la pantalla (coordenadas gráficas)
  int ax= 1, ay= 1;	// Coordenadas de la anterior casilla seleccionada
  int terminar = 0;
  int tecla;
  ...
  do
  {
    // Borramos el recuadro de su posición anterior redibujando toda la pantalla
    ...
    // Mostramos el recuadro de selección actual. Las coordenadas son (x,y)
    px = ((x-1)*TAM_CASILLA) + DESPLZ_ORIG_X;
    py = ((y-1)*TAM_CASILLA) + DESPLZ_ORIG_Y;
    rect = (SDL_Rect) { px, py, TAM_CASILLA, TAM_CASILLA };
    if (x_orig==0)	// Usamos un sprite distinto según se esté eligiendo casilla origen o destino
    ...
    // Enviamos la imagen del recuadro a la memoria de vídeo para que se vea
    SDL_Flip(scr);
...
void cargar_imagenes()
{
  ...
  // Cargamos la imagen del RECUADRO de selección 1
