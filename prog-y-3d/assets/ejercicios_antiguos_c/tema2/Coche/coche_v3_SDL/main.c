...
int main(int argc, char *argv[])
{
   ...
   while (salir == 0)
   {
      ...
      SDL_Flip(pantalla);   // Refresca memoria de vídeo
   }
   finalizar_SDL();
   return 0;
}
...
void iniciar_SDL()
{
  ...
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) == -1) {
    puts("Error en la inicialización del sistema de vídeo y/o audio\n");
    SDL_Quit();
  }
  ...
  // Establece atributos del ratón y del teclado
  SDL_ShowCursor(SDL_DISABLE);  // Oculta el ratón
  SDL_EnableKeyRepeat(1, 1);    // Activa repetición de teclas
  // Carga las imágenes
...
void inicializar_variables(struct s_posicion disparos[MAX_DISPAROS], struct s_posicion estrellas[NUM_ESTRELLAS], struct s_posicion enemigos[MAX_ENEMIGOS], struct s_posicion *posic_jugador)
{
    ...
    // Inicializa enemigos
    for (i = 0; i<10; i++)  // Sólo ponemos 10 (por ahora)
    {
        enemigos[i].x = rand()%600+100;
        enemigos[i].y = -50;
        enemigos[i].tiempo = rand()%500;
...
int mover_jugador(struct s_posicion *jugador, struct s_posicion disparos[MAX_DISPAROS])
{
    ...
    switch(ev.type)
    {
        case SDL_QUIT:  salir = 1;
                        break;
        case SDL_MOUSEMOTION:    // Se ha movido el ratón
                        jugador->x = ev.motion.x;
                        jugador->y = ev.motion.y;
                        break;
        case SDL_MOUSEBUTTONDOWN: // Se ha hecho clic con el ratón
        ...
        if (disparos[i].estado == ACTIVO)
        {
            ...
            for (j=0; j<MAX_ENEMIGOS; j++) {
                 ...
                 if ((abs(disparos[i].x - enemigos[j].x) < 40) && (abs(disparos[i].y - enemigos[j].y) < 60) && (enemigos[j].estado == ACTIVO))
                 {
                     ...
                     // Lanzamos el sonido de la explosión
...
void mover_enemigos(struct s_posicion enemigos[MAX_ENEMIGOS], int tiempo)
{
    ...
    {
        // Enemigo en espera: comprobar si le toca ya salir a escena (activación)
