...
int pintar_tablero()					// Solo pinta las casillas
{
...
int inicializar_tablero(struct s_casilla tablero[9][9]) // inicializa la matriz interna que gestiona el tablero
{

	int x,y,newx,newy;
	char color='N';

for (x=0;x<=8;x++)         	// identifica el color de cada casilla
	{
	for (y=0;y<=8;y++)
		{
		if (color=='N')
...
for (x=1;x<=8;x++)             // Pone todas las casillas del tablero a ' '
	{
	for (y=1;y<=8;y++)
		{
		tablero[x][y].figura=' ';
...
for (x=0;x<=8;x++)             // Coloca las fichas
	{
	for (y=1;y<=8;y++)
		{
		if (x==1)
...
int refrescar_matriz(struct s_casilla tablero[9][9]) // Pinta las fichas en el tablero segun su posicion en la matriz interna
{
	int x,y,newx,newy;
