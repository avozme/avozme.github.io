...
### 2.8.1. Arquitectura de aplicaciones
...
### 2.8.2. ¿Qué es la arquitectura MVC?
...
### 2.8.3. MVC en la práctica: una implementación incremental

<div style='color:red'><i>ATENCIÓN: esta sección es solo para quien esté interesado en comprender más profundamente el patrón MVC. Léela atentamente si es tu caso. Si no, puedes saltar sin problemas al <a href='../mvc/#174-el-patr%C3%B3n-mvc-en-la-teor%C3%ADa'>Apartado 1.8.4</a></i>.</div>
...
// 1. Lógica: Conexión y consulta usando PDO
$dsn = "mysql:host=mi-host;dbname=mi-base-de-datos;charset=utf8mb4";
$pdo = new PDO($dsn, 'mi-usuario', 'mi-clave', [
    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION
]);

$stmt = $pdo->query('SELECT fecha, titulo FROM articulo');
?>

<!-- 2. Presentación: Generamos la tabla HTML -->
...
```php
<?php
  // Este es el controlador (index.php)
  $dsn = "mysql:host=mi-host;dbname=mi-base-de-datos;charset=utf8mb4";
  $pdo = new PDO($dsn, 'mi-usuario', 'mi-clave');
...
La **vista** (*showAllArticles.php*) ahora es mucho más limpia: 
...
$articulos = Articles::getAll();   // Este método del modelo nos devuelve la lista de artículos
...
class Articles {
    /**
     * Recupera todos los artículos de la BD
     * @return array Array bidimensional con los artículos
     */
...
De ese modo, cuando queramos hacer una consulta desde el modelo, no lo haremos con los métodos nativos de la base de datos (como *query()*, *fetch()* y similares), sino con los nuestros (*consultar()*, *manipular()*, o como los hayamos querido llamar).
...
class Db {
    ...
    /**
     * Abre la conexión con la base de datos usando PDO
     */
    public function createConnection(string $host, string $user, string $pass, string $dbname): void {
    ...
    /**
     * Lanza una consulta (SELECT)
     */
    public function dataQuery(string $sql, array $params = []): array {
    ...
    /**
     * Lanza una sentencia de manipulación (INSERT, UPDATE, DELETE)
     */
    public function dataManipulation(string $sql, array $params = []): int {
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
...
class Articles {
    public function getAll(): array {
        ...
        // Consultamos a través de la capa de abstracción
...
#### Cuarta mejora: Estructura profesional con Composer y Namespaces

En el desarrollo profesional, no andamos haciendo `include` manuales de decenas de archivos. En su lugar, utilizamos **Composer** y el estándar **PSR-4** para la carga automática de clases (*autoloading*).

Esta es la estructura de carpetas que debería tener una aplicación PHP moderna y profesional:

```text
mi-proyecto/
├── src/                # Código fuente con namespaces
│   ├── Controllers/
...
│   └── index.php
├── vendor/             # Librerías (gestionado por Composer)
└── composer.json       # Configuración de Composer
```

**1. Configuración de Composer (`composer.json`)**

Primero, definimos cómo se cargarán nuestras clases automáticamente:
...
Después de crear este archivo, ejecutamos `composer install` (o `composer dump-autoload`) y ¡listo! Ya no necesitamos más `include`.

**2. El Modelo profesional (`src/Models/Article.php`)**

Utilizamos **namespaces** para organizar el código y **PDO** para la seguridad:
...
**3. El Controlador profesional (`src/Controllers/ArticleController.php`)**
...
class ArticleController {
    ...
    public function index() {
        $model = new Article($this->pdo);
        $articles = $model->getAll();
        
        // Simplemente pasamos los datos a la vista
        require __DIR__ . '/../Views/articles_index.php';
    }
}
```

**4. El Front Controller (`public/index.php`)**

Este es el único archivo accesible desde la web. Se encarga de arrancar la aplicación:
...
// Enrutamiento básico (esto lo haría un Router en un sistema real)
...
Esta arquitectura es la base sobre la que se asientan frameworks profesionales como **Laravel** o **Symfony**. Al usar *namespaces* y *autoloading*, tu código es mucho más fácil de mantener, testear y escalar.

#### Por qué usar Inyección de Dependencias (DI)

En el ejemplo anterior, tanto `Article` como `ArticleController` reciben el objeto `PDO` en el constructor:

```php
public function __construct(private PDO $pdo) {}
```

Esto es **Inyección de Dependencias**. En lugar de que la clase "fabrique" o "busque" su propia conexión (lo cual la haría dependiente de una configuración global), la conexión le es "inyectada" desde fuera.

**Ventajas:**
1. **Testabilidad:** Podemos pasar un objeto "mock" (falso) para probar la clase sin base de datos real.
2. **Flexibilidad:** Si cambiamos la configuración de la BD, solo la cambiamos en el punto de entrada (`index.php`).
3. **Desacoplamiento:** La clase `Article` no sabe ni le importa cómo se creó el objeto `PDO`. Solo sabe que lo necesita.

En sistemas grandes, usamos un **Contenedor de Dependencias (Service Container)** que se encarga de instanciar todas estas clases y pasarles lo que necesitan automáticamente.

### 2.8.4. El patrón MVC en la teoría
...
*(Si no has leído el ejemplo práctico del [Apartado 1.8.3](../mvc/#173-mvc-en-la-práctica-una-implementación-incremental), es posible que las siguientes definiciones no acaben de tener sentido para ti. No te preocupes, lo irás pillando más adelante conforme practiques con el patrón MVC).*
...
* **Los controladores**, donde se captura cada petición del usuario y se dirige el flujo de ejecución, invocando a los modelos y a las vistas en el orden adecuado.
   ...
   Los controladores estarán compuestos por una colección de métodos, uno para cada funcionalidad de la aplicación. El método que se ejecute en cada ocasión estará controlado por la URL. En concreto, por la variable *"action"* que se pasará por GET, aunque, por supuesto, puedes ponerle otro nombre si *"action"* no te gusta.
    ...
    En los *frameworks*, esta variable "action" se transforma en una URL limpia que, a través de un objeto adicional llamado **enrutador**, termina provocando la invocación del método adecuado. Muchas veces, esta asociación entre URL y método se hace mediante **Atributos de PHP 8**, unas etiquetas especiales que se ponen encima de los métodos para indicar qué ruta deben atender. 
...
### 2.8.6. Una implementación completa: Biblioteca

A continuación encontrarás una posible implementación de una Biblioteca hecha con PHP clásico, sin ningún framework. La implementación sigue la arquitectura MVC, pero ten en cuenta que **solo es una primera aproximación que se puede mejorar en muchos aspectos**, como, por ejemplo:
