...
int menu(void);
void insertar_datos(int *n_regs);
...
int main(void)
{
   ...
   do
   {
      ...
      switch (opc)
      {
        case 1: buscar_por_codigo(); break;
        case 2: insertar_datos(&n_regs); break;
...
int menu(void)
{
   ...
   do
   {
     ...
     gotoxy(22,7);  printf("+-------------------------------+\n");
     gotoxy(22,8);  printf("|      Buscar película          |\n");
     gotoxy(22,9);  printf("|      Añadir película          |\n");
     gotoxy(22,10); printf("|      Borrar película          |\n");
     gotoxy(22,11); printf("|      Modificar película       |\n");     
     gotoxy(22,12); printf("|      Listar películas         |\n");     
     gotoxy(22,13); printf("|      Mostrar índice           |\n");
     gotoxy(22,14); printf("|      Reconstruir índice       |\n");
     gotoxy(22,15); printf("|      Salir                    |\n");
     gotoxy(22,16); printf("+-------------------------------+\n");
     ...
     switch(opc)
     {
       case 1:   printf("|      Buscar película          |\n"); break;
       case 2:   printf("|      Añadir película          |\n"); break;
       case 3:   printf("|      Borrar película          |\n"); break;
       case 4:   printf("|      Modificar película       |\n"); break;
       case 5:   printf("|      Listar películas         |\n"); break;
       case 6:   printf("|      Mostrar índice           |\n"); break;
       case 7:   printf("|      Reconstruir índice       |\n"); break;
       case 8:   printf("|      Salir                    |\n"); break;
     }
   
     // Leer el teclado y actualiza la opción activada
     tecla = getch();
     if (tecla == '+') { opc++; if (opc == 9) opc = 1; }   
     if (tecla == '-') { opc--; if (opc == 0) opc = 8; }
   }
...
// Añade un registro al archivo de datos. Primero, intentará buscar un
// hueco en el segmento adecuado. Si no, lo argregará al archivo de excedentes.
void insertar_datos(int *n_regs)
{
    FILE *f, *f_ind;
    ...
    // Buscamos el segmento en el que la película encajaría
    while ((!feof(f_ind)) && (entrada_indice.clave_ultimo < cod_buscado))
        fread(&entrada_indice, sizeof(struct s_indice), 1, f_ind);

    if (feof(f_ind))
        // Se llegó al final del índice sin localizar el segmento adecuado
        // para la película. Eso significa que el código de película es el 
        // mayor de todos, y que debe insertarse al final del archivo de datos.
        insertar_datos_final(n_regs);
    else
    {
    }
}
...
void borrar_pelicula(void)
{
    printf("Función sin implementar\n");
}
