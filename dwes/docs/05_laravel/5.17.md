...
permalink: /laravel/crud-con-laravel
nav_order: 17
has_children: false
parent: 5 Laravel
...
El seeder se crea con el comando ```php artisan make:seeder ProductTableSeeder```, eque generará el archivo ***/database/seeders/ProductTableSeeder.php***.
...
@section("content")
    <a href="{% raw %}"{{ route('product.create') }}{% endraw %}"">Nuevo</a>
    <table border='1'>
    @foreach ($productList as $product)
        <tr>
            ...
            <td>
                <a href="{% raw %}"{{route('product.edit', $product->id)}{% endraw %}"}">Modificar</a></td>
            <td>
                <form action = "{% raw %}"{{route('product.destroy', $product->id)}{% endraw %}"}" method="POST">
                    @csrf
                    @method("DELETE")
...
@section("content")
    @isset($product)
        <form action="{% raw %}"{{ route('product.update', ['product' => $product->id]) }}{% endraw %}"" method="POST">
        @method("PUT")
    @else
        <form action="{% raw %}"{{ route('product.store') }}{% endraw %}"" method="POST">
    @endisset
        @csrf
        Nombre del producto:<input type="text" name="name" value="{% raw %}"{{$product->name ?? '' }}{% endraw %}""><br>
        Descripción:<input type="text" name="description" value="{% raw %}"{{$product->description ?? '' }}{% endraw %}""><br>
        Precio:<input type="text" name="price" value="{% raw %}"{{$product->price ?? '' }}{% endraw %}""><br>
        <input type="submit">
        </form>
@endsection
```
...
* Se puede simplificar el método *store()* del controlador haciendo una asignación masiva. Bastaría con indicar en el modelo que todos los campos de la tabla son *fillables* para poder hacer en el método *store()* del controlador algo como esto: ```$p = new Producut($r->all())```
