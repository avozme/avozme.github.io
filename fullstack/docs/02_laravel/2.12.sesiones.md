...
$ ./vendor/bin/sail npm run dev   # En producción, usar npm run build
```

***IMPORTANTE**: no escribas ```./vendor/bin/sail``` en los prompts anteriores si no estás usando Laravel Sail*.
...
La instalación de Breeze en Laravel 12 se hace en modo interactivo y te hará varias preguntas:

* Elige "**Blade with Alpine**". Es la forma clásica de Laravel Breeze, con vistas generadas en el backend, que es como estamos trabajando con Laravel por ahora.
* Elige "**Dark mode support**" si quieres tener la opción de poner la pantalla de login en "modo oscuro".
* Elige el "**Testing framework"** que quieras (es indiferente, puesto que de momento no estamos haciendo testing). 

Una vez hecho esto, *Breeze* creará automáticamente varias rutas en ```/routes/web.php```. **¡¡OJO, PERDERÁS TU web.php, ASÍ QUE HAZ UNA COPIA ANTES!!**. Entre las rutas nuevas que encontrarás están estas:
...
Por último, se crea una vista *home* de ejemplo (llamada ```dashboard.blade.php```) a la que llegamos después de hacer login. Esa vista *home* la puedes cambiar en ```/app/Http/Controller/Auth/AuthenticatedSessionController.php``` (método ```store()```) para redirigirla a la vista que te interese en lugar de *dashboard*.
...
Podemos usar estos *middlewares* en nuestro enrutador, es decir, en ```/routes/web.php``` para proteger los endpoints que nos interese, es decir, para impedir el acceso de usuarios no logueados a ciertos métodos de los controladores.

Para lograrlo, basta con que añadas ```->middleware('auth')``` al final del línea que quieres proteger en tu enrutador:

```php
   // Vamos a proteger la ruta /ruta-a-proteger
