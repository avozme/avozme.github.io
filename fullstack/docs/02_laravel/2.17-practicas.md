...
En la siguiente colección de actividades vamos a trabajar con esta base de datos:
<ul>
<li><strong>Tabla users</strong>: id, username, email, password, role.</li>
<li><strong>Tabla articles</strong>: id, title, body, date, user_id.</li>
</ul>
</div>
...
* **Tareas**:
    ...
    4. Lanza las migraciones que Laravel trae "de fábrica" (son necesarias para que Laravel funcione): `$ ./vendor/bin/sail artisan migrate` o `$ php artisan serve`.
    5. Explora la estructura de directorios. [Más info aquí](../laravel/configuracion.html#231-estructura-de-directorios).
...
* **Tareas**:
    ...
    2. **Crea las migraciones** para las tablas `users` y `articles`, si es que aún no las has creado. [Más info aquí](../laravel/migraciones.html#291-crear-tablas-mediante-las-migraciones).
        ...
        ***¡¡OJO!!** La migración de `users` ya existe en Laravel. Esta no tendrás que crearla, sino modificarla. Es importante que no la elimines, sino que añadas en ella lo que necesites. Si la eliminas o la reescribes por completo, Laravel no funcionará bien.*

    3. **Crea *seeders* para ambas tablas** para introducir algunos datos mínimos de prueba. [Más info sobre los seeders aquí](../laravel/migraciones.html#294-seeding).
    4. **(OPTATIVO) Rellena las tablas con *Faker***. Esto te permitirá crear datos ficticios realistas sin esfuerzo. [Más info sobe Faker aquí](../laravel/migraciones.html#295-faker-rellenar-las-tablas-con-datos-realistas)
    5. **Lanza las migraciones y los seeders** con `$ ./vendor/bin/sail artisan migrate:fresh --seed`.
